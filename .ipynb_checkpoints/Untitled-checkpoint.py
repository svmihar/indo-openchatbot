...
import joblib
df = pd.read_csv('./data/final_es_conv.csv')
df_tian = pd.read_csv('./data/conversation_indo_formatted.csv')
# df_tian = pd.DataFrame(joblib.load('./data/conv_preprocessed.pkl'))


# In[79]:


# a = joblib.load('./data/conv_preprocessed.pkl')


# In[72]:


# df_tian = pd.Series([' '.join(x) for x in a])
...
trn_tian, val_tian = train_test_split(df_tian, test_size=.3)
...
a = trn_df[df.columns].apply(lambda x: ' '.join(x.astype(str)), axis = 1)
b = trn_tian[df.columns].apply(lambda x: ' '.join(x.astype(str)), axis = 1)
...
lens, tok_cnt, word_cnt = get_counter_and_lens(b, tokenizer)
